...
SPLIT_MODE       = "stratified"    # "chronological" (by Time, 60/20/20) or "stratified"
...
def estimate_eps(X, min_samples=10, sample_cap=20_000, plot_path="dbscan_kdistance_random.png"):
...
def plot_curves(y_true, curves):
    ...
    plt.savefig("pr_curves_random.png", dpi=150); plt.close()
    ...
    plt.savefig("roc_curves_random.png", dpi=150); plt.close()
...
def main():
    ...
    print(f"  eps = {eps:.4f}  (plot -> dbscan_kdistance_random.png)\n")
    ...
    df.to_csv("benchmark_results_random.csv", index=False)
    print("\nSaved -> benchmark_results_random.csv")

    # ---- PR and ROC curves (one figure each, both protocols overlaid) ---- #
    plot_curves(yte, curves)
    print("Saved -> pr_curves_random.png, roc_curves_random.png")
